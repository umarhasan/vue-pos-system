...
class PurchasesController extends BaseController
{
    ...
    public function index(request $request)
    {
        ...
        $Purchases = Purchase::with('details','details.product','facture', 'provider', 'warehouse')
        ...
        \DB::transaction(function () use ($request) {
            ...
            foreach ($data as $key => $value) {
                ...
                if ($value['product_variant_id'] !== null) {
                    ...
                    $product_update_cost = ProductVariant::where('product_id', $value['product_id'])
                    ->where('id', $value['product_variant_id'])
                    ->where('deleted_at', '=', null)
                    ->first();

                    $product_update_cost->cost = $value['Unit_cost'];
                    $product_update_cost->save();

                }else{

                    $product_update_cost = Product::where('deleted_at', '=', null)
            ...
            if (PurchaseReturn::where('purchase_id', $id)->where('deleted_at', '=', null)->exists()) {
                ...
                foreach ($new_purchase_details as $key => $prod_detail) {
                    ...
                    if ($prod_detail['product_variant_id'] !== null) {
                        ...
                        $product_update_cost = ProductVariant::where('product_id', $prod_detail['product_id'])
                        ->where('id', $prod_detail['product_variant_id'])
                        ->where('deleted_at', '=', null)
                        ->first();

                        $product_update_cost->cost = $prod_detail['Unit_cost'];
                        $product_update_cost->save();
    ...
    public function Send_Email(Request $request)
    {
        ...
        $email['Ref'] = $invoice_number;

        $pdf = $this->Purchase_pdf($request, $request->id);

        $this->Set_config_mail();
        $mail = Mail::to($receiver_email)->send(new PurchaseMail($email, $pdf));
        return $mail;
    }
