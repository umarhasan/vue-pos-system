...
class ProductsController extends BaseController
{
    ...
    public function index(request $request)
    {
        ...
        // Search with multiple parameters in the product name and note
        $searchQuery = $request->input('search');
        if ($request->filled('search')) {
            $tokens = explode(' ', $searchQuery);

            $Filtred = $Filtred->where(function ($query) use ($tokens) {
                foreach ($tokens as $token) {
                    $pattern = "%{$token}%";
                    $query->where(function ($q) use ($pattern) {
                        $q->where('products.name', 'LIKE', $pattern)
                            ->orWhere('products.note', 'LIKE', $pattern)
                            ->orWhere('products.code', 'LIKE', $pattern);
                    });
                }
            });
        }
        ...
        $Filtred->orWhere(function ($query) use ($request) {
    ...
    public function Products_by_Warehouse(request $request, $id)
    {
        ...
        foreach ($product_warehouse_data as $product_warehouse) {
            ...
            if ($product_warehouse->product_variant_id) {
                ...
                $item['qty'] = $product_warehouse['productVariant']->qty;
                ...
                $item['qty'] = $product_warehouse['product']->qty;
            ...
            $item['note'] = $product_warehouse['product']->note;
    ...
    public function Get_element_barcode(Request $request)
    {
        ...
        return response()->json(['warehouses' => $warehouses,'show_product_name' => $show_product_name]);
