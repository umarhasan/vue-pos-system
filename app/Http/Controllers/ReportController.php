...
class ReportController extends BaseController
{
    ...
    public function product_report(request $request)
    {
        ...
        $products_data = Product::where('deleted_at', '=', null)->select('id', 'name','code', 'is_variant','unit_id','type','note');
         ...
         if ($request->filled('search')) {
             ...
             $products_data = $products_data->where(function ($query) use ($tokens) {
                 foreach ($tokens as $token) {
                     $query->where(function ($q) use ($token) {
                         $q->where('products.name', 'LIKE', "%{$token}%")
                             ->orWhere('products.note', 'LIKE', "%{$token}%")
                             ->orWhere('products.code', 'LIKE', "%{$token}%");
                     });
                 }
             });
         }
